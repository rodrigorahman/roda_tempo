...
void main() => runApp(
      MaterialApp(
        theme:
            ThemeData.light().copyWith(scaffoldBackgroundColor: primaryAccent),
...
class TimeOfDayWidget extends LeafRenderObjectWidget {
  ...
  void updateRenderObject(
      BuildContext context, covariant TimeOfDayRenderBox renderObject) {
    renderObject
      ..value = value
      ..onChanged = onChanged;
  }
...
class TimeOfDayRenderBox extends RenderBox {
  ...
  BoxConstraints get constraints =>
      super.constraints.copyWith(maxHeight: 40, maxWidth: 80);
  ...
  void _animate(Offset pixelsPerSecond, int index) {
    ...
    final tween = Tween(begin: _selectedCenter, end: finalOffset)
  ...
  void paint(PaintingContext context, ui.Offset offset) {
    ...
    final rrect = RRect.fromRectAndRadius(
      Rect.fromLTRB(
          rect.left, rect.top - deltaDy, rect.right, rect.bottom + deltaDy),
  ...
  void _drawParagraph(Canvas canvas, String text,
    ...
    final paragraphConstraints =
        ui.ParagraphConstraints(width: fontSize * text.length);
...
class DialWidget extends LeafRenderObjectWidget {
  ...
  @override
  void updateRenderObject(
      BuildContext context, covariant DialRenderBox renderObject) {
    renderObject
      ..onChanged = onChanged
      ..time = time;
  }
...
class DialItemRenderBox extends RenderBox {
  ...
  void _animate(Offset pixelsPerSecond) {
    ...
    final tween = Tween(begin: _currentAngle, end: restingAngle)
        .chain(CurveTween(curve: Curves.easeIn));
    _animation = _controller.drive(tween);
  ...
  void _onDragUpdate(DragUpdateDetails details) {
    ...
    final diffInAngle = toCartersian(previousOffset, radius) -
        toCartersian(_currentDragOffset, radius);
  ...
  void onLayout(ui.Size size, ui.Offset offset) {
    ...
    circularPath = Path()
      ...
      ..relativeArcToPoint(Offset(-innerRadius * 2, 0),
  ...
  void paint(PaintingContext context, ui.Offset offset) {
    ...
    for (int i = 0; i < divisions; i++) {
      ...
      final currentAngle =
          startingAngleOffset - ((i * totalAngle / divisions) + _currentAngle);
      final startingOffset =
          toPolar(center, currentAngle, radius - fontSize - padding);
...
class DialRenderBox extends RenderBox
    with ContainerRenderObjectMixin<DialItemRenderBox, DialParentData> {
  DialRenderBox({
  ...
  void performLayout() {
    ...
    // TODO: for some weird reason, this breaks on flutter web.
    // Do resize the flutter window to a mobile device
    // final firstChildRadius = math.min(tempFirstChildRadius, 240.0);
    final firstChildRadius = tempFirstChildRadius;
    ...
    while (child != null) {
      ...
      final childRadius = previousRadius != null
          ? previousRadius + childHeight
          : firstChildRadius - horizontalOffset;
      final childSize = Size.fromRadius(childRadius);
      final center = Offset(firstChildRadius, firstRadius ?? childRadius);

      final DialParentData childParentData = child.parentData;
      childParentData.offset =
          Offset(center.dx - childRadius, center.dy - childRadius);
    ...
    size = Size(
        ...
        firstRadius +
            previousRadius +
            (DialItemRenderBox.indicatorHeight * 5 / 2));
  }
